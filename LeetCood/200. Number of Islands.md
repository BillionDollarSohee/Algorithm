...
## 코틀린 풀이
같은 코드를 코틀린으로 풀면 2가지를 개선할 수 있다.
1. 조건문을 when으로 일목요연하게 정리
2. 중첩함수를 이용하여 매번 그래프 구조를 grid라는 변수에 담아 전달하지 않을 수 있다.
(dfs()가 numIslands()내에 위치하므로 grid변수를 공유할 수 있기 때문에)

```kotlin
class Solution {
    fun numIslands(grid: Array<CharArray>): Int {
        fun dfs(i: Int, j: Int) {
            // 현재 위치가 그리드 밖이거나 물이면 종료
            when {
                (i < 0) ||
                (i >= grid.size) ||
                (j < 0) ||
...
```
요즘 리트코드가 발전했는지 코틀린 결과가 더 좋게 나올때가 많아진 것 같다.
<img width="687" height="552" alt="image" src="https://github.com/user-attachments/assets/79a7b09a-5d70-47b9-a0cc-bccb56300281" />
