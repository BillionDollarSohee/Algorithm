...
원형으로 구현하려는 이유는 엘리먼트를 채우다가 공간이 다 차면 tail과 head를 연결하여 앞 쪽의 빈 공간을 활용하려는 의도인데, 
연결리스트는 애초에 빈 공간이라는 개념이 존재하지 않기 때문에 원형으러 구현하는 것이 의미가 없다.
데크의 연산은 맨 처음과 맨 끝의 값을 추출할 뿐이며 맨 끝 다음 값을 추출하는 연산은 없기 때문에 연결될 필요 또힌 없다.
이 문제는 그저 구현을 해보며 익히는데 도움을 주겠지만 실무에서는 쓰이지 않을 방식인 것이다.
...
또 한가지 차이점은 this 선언 여부가 있다.
원래는 this를 매번 기입해야 하는 것이 맞다.
하지만 메서드 변수가 많은 경우 매번 this를 기입하면 코드가 무척 지저분해 보인다.
자바에서는 굳이 this를 지정하지 않아도 내부에 선언된 변수가 없으면 자동으로 클래스 속성을 뒤지므로 변수명이 중복되는 경우만 아니라면 this를 생략해도 무방하다.
상황에 맞게 생략하면 가독성이 높아질 수 있으니 적절히 활용하면 좋다.
